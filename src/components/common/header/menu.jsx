const menuData = [
  {
    id: 1,
    title: "Home",
    path: "/",
    newTab: false,
  },
  {
    id: 2,
    title: "About Us",
    newTab: false,
    submenu: [
      {
        id: 21,
        title: "Dr. Sunil Prakash",
        path: "/about/dr-sunil-prakash",
        newTab: false,
      },
      {
        id: 22,
        title: "Certificates",
        path: "/about/certificates",
        newTab: false,
      },
      {
        id: 23,
        title: "Case Studies",
        path: "/case-studies",
        newTab: false,
      },
      {
        id: 24,
        title: "Editor DNS News Letter",
        path: "/editor-dns-news-letter",
        newTab: false,
      },
      {
        id: 25,
        title: "Countries Visited for Fellowship CME's",
        path: "/countries-visited-for-fellowship-cmes",
        newTab: false,
      },
    ],
  },
  {
    id: 3,
    title: "Treatments",
    newTab: false,
    submenu: [
      {
        id: 31,
        title: "Renal Replacement Surgery",
        path: "/renal-replacement-surgery",
        newTab: false,
      },
      {
        id: 32,
        title: "Kidney Transplantation",
        path: "/kidney-transplantation",
        newTab: false,
      },
      {
        id: 33,
        title: "Dialysis",
        path: "/dialysis",
        newTab: false,
      },
      {
        id: 34,
        title: "Slowing of Kidney Disease by DIET",
        path: "/slowing-of-kidney-disease-by-diet",
        newTab: false,
      },
      {
        id: 35,
        title: "Home Dialysis Treatments",
        path: "/home-dialysis-treatments",
        newTab: false,
      },
      {
        id: 36,
        title: "Hemodialysis Treatments for Kidney",
        path: "/hemodialysis-treatments-for-kidney",
        newTab: false,
      },
      {
        id: 37,
        title: "Kidney Failure Treatments",
        path: "/kidney-failure-treatments",
        newTab: false,
      },
      {
        id: 38,
        title: "Patient Education Diabetes",
        path: "/patient-education-diabetes",
        newTab: false,
      },
      {
        id: 39,
        title: "Kidney Donation",
        path: "/kidney-donation",
        newTab: false,
      },
    ],
  },
  {
    id: 4,
    title: "Media Coverage",
    path: "/media-coverage",
    newTab: false,
  },
  {
    id: 5,
    title: "Happy Patients",
    submenu: [
      {
        id: 51,
        title: "Success Stories",
        path: "/success-stories",
        newTab: false,
      },
      {
        id: 52,
        title: "Testimonials",
        path: "/testimonials",
        newTab: false,
      },
      {
        id: 53,
        title: "Video Reviews",
        path: "/dialysis",
        newTab: false,
      },
    ],
    newTab: false,
  },
  {
    id: 6,
    title: "Gallery",
    submenu: [
      {
        id: 61,
        title: "Photo Gallery",
        path: "/gallery/photoGallery",
        newTab: false,
      },
      {
        id: 62,
        title: "Awards",
        path: "/gallery/awards",
        newTab: false,
      },
    ],
    newTab: false,
  },
  {
    id: 7,
    title: "Information",
    path: "/information",
    newTab: false,
  },
  {
    id: 8,
    title: "Blog",
    path: "/blogs",
    newTab: false,
  },
  {
    id: 7,
    title: "Contact Us",
    path: "/contact-us",
    newTab: false,
  },
];
export default menuData;
