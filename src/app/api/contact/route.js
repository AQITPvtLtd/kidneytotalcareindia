...
export async function POST(request) {
  try {
    ...
    {
      console.log({ name, phone, email, date, message });
    }

    await new Promise((resolve, reject) => {
      connection.query(
        "INSERT INTO contact(id, name, phone, email, date, message) VALUES (?,?,?,?,?,?)",
        [unique_id, name, phone, email, date, message],
        (err, results, fields) => {
          if (err) {
            console.error(err);
            reject(err);
      ...
      html: `<html lang="en">
        ...
        <body>
          <div class="img-container" style="display: flex;justify-content: center;align-items: center;border-radius: 5px;overflow: hidden; font-family: 'helvetica', 'ui-sans';"></div>
                ...
                <h3>You've got a new mail from ${name}, their email is: ✉️${email}, Phone Number is: ${phone} and Date is ${date}</h3>
